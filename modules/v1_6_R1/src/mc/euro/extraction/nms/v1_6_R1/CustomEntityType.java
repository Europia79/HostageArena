...
try {
((Map) getPrivateStatic(EntityTypes.class, "c")).remove(entity.getCustomClass()); // d.put(paramClass, paramString);
} catch (Exception e) {
e.printStackTrace();
}
 
try {
((Map) getPrivateStatic(EntityTypes.class, "e")).remove(entity.getCustomClass()); // f.put(paramClass, Integer.valueOf(paramInt));
} catch (Exception e) {
e.printStackTrace();
}
}
